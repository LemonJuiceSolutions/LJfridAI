...
import { Check, Send, Printer, ClipboardList, Play, ChevronRight, Scissors, AlertTriangle, CheckCircle, Percent, Package } from 'lucide-react';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export type Job = {
    ...
    supplier?: string;
    bom: { components: { name: string; quantity: number; unit: string }[] };
    deliveryDate: string;
...
const getInitialJobStatus = (jobId: string): { status: JobStatus; assignedTo: 'Internal' | 'External' | null, supplier?: string } => {
    const numericId = parseInt(jobId.replace('JOB', ''), 10);
...
const getReturnDate = (jobId: string) => {
    ...
    const numericId = parseInt(baseJobId.replace('JOB', ''), 10);
...
const CuttingSheetDialog = ({ job, isOpen, onOpenChange }: { job: Job | null, isOpen: boolean, onOpenChange: (open: boolean) => void }) => {
    if (!job) return null;

    const isExternal = job.assignedTo === 'External';
    const documentTitle = isExternal ? 'Ordine di Conto Lavoro / DDT' : 'Scheda di Lavorazione Taglio';
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mt-2">
                        ...
                        {isExternal && (
                             <p><strong>DDT Uscita N°:</strong> DDT-OUT-{parseInt(job.jobId.replace(/\D/g, ''))}</p>
                        )}
                    </div>
                </div>
                <div>
...
export default function CuttingWidget() {
    ...
    const [jobs, setJobs] = useState<Job[]>(() => {
            ...
            order.lines
                ...
                .map(line => {
                    ...
                    return {
                        ...
                        ddtNumber: finalStatus.status.startsWith('Returned') ? `DDT-R-${parseInt(line.jobId.replace('JOB',''))}` : null,
                        returnedQuantity: isCompletedOrReturned ? previousStageQuantity - (parseInt(line.jobId.replace('JOB','')) % 5) : null, // Simulate a small loss
        ...
        const statusesToEnsure = ['To be assigned', 'To be prepared', 'In Progress', 'Awaiting Return'];
        statusesToEnsure.forEach(statusPrefix => {
            while (allJobsFromOrders.filter(j => j.status.startsWith(statusPrefix)).length < 2) {
                const jobToConvert = allJobsFromOrders.find(j => 
                    statusPrefix === 'To be assigned' ? !j.status.startsWith('To be') :
                    statusPrefix === 'To be prepared' ? j.status === 'To be assigned' :
                    j.status.startsWith('To be prepared')
                );
                
                if (jobToConvert) {
                    if (statusPrefix === 'To be assigned') {
                        // This case is unlikely if we filter correctly, but as a fallback
                         jobToConvert.status = 'To be assigned';
                         jobToConvert.assignedTo = null;
                         jobToConvert.supplier = undefined;
                    } else if (statusPrefix === 'To be prepared') {
                        jobToConvert.status = 'To be prepared (Internal)';
                        jobToConvert.assignedTo = 'Internal';
                    } else { // In Progress or Awaiting
    ...
    const handleMarkAsComplete = (jobId: string, ddt?: string) => {
        setJobs(prevJobs => 
            prevJobs.map(job => {
                if (job.jobId === jobId) {
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Qtà da Lavorare</TableHead>
  ...
  return (
    ...
    <CardHeader>
      <CardTitle>Reparto Taglio</CardTitle>
      <CardDescription>Assegna e traccia i lotti di taglio per lavorazioni interne ed esterne.</CardDescription>
    ...
    {selectedJob && (
     ...
     <CuttingSheetDialog job={selectedJob} isOpen={sheetDialogOpen} onOpenChange={setSheetDialogOpen} />
    </Card>
  );
}