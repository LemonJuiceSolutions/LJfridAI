...
export default function VisualTree({ treeData, onDataRefresh, isSaving: parentIsSaving, initialNodePath }: VisualTreeProps) {
    ...
    useEffect(() => {
        if (treeData?.id) {
            getTreeSchedulesAction(treeData.id).then(res => {
                if (res.success && res.data) {
                    setScheduledNodeIds(new Set(res.data));
                }
            });
        }
    }, [treeData?.id]);
    ...
    const handleSavePreview = async (path: string, previewData: any) => {
        ...
        try {
            ...
            if (previewData.sqlPreviewData) {
                ...
                // Preserve existing plotlyStyleOverrides/htmlStyleOverrides if not included in new preview data
                const existingOverrides = currentNode?.pythonPreviewResult?.plotlyStyleOverrides;
                const existingHtmlOverrides = currentNode?.pythonPreviewResult?.htmlStyleOverrides;
                console.log('[DEBUG] Salvataggio Python preview data:', { type: previewData?.type, hasData: !!previewData?.data, hasNewOverrides: !!previewData?.plotlyStyleOverrides, hasExistingOverrides: !!existingOverrides, hasNewHtmlOverrides: !!previewData?.htmlStyleOverrides, hasExistingHtmlOverrides: !!existingHtmlOverrides });
                updatedNodeData.pythonPreviewResult = {
                    ...
                    ...(existingHtmlOverrides && !previewData.htmlStyleOverrides ? { htmlStyleOverrides: existingHtmlOverrides } : {}),
                };
            }
            ...
            setTree(prevTree => {
                ...
                if (nodeToUpdate) {
                    if (previewData.sqlPreviewData) {
                        ...
                        const existingLocalHtmlOverrides = nodeToUpdate.pythonPreviewResult?.htmlStyleOverrides;
                        nodeToUpdate.pythonPreviewResult = {
                            ...
                            ...(existingLocalHtmlOverrides && !previewData.htmlStyleOverrides ? { htmlStyleOverrides: existingLocalHtmlOverrides } : {}),
                        };
                    }
                    ...
                    if (isAncestor) {
                        ...
                        if (actualNode.sqlResultName) {
                            ...
                            ancestorItems.push({
                                ...
                                data: actualNode.sqlPreviewData,
                                aiConfig: actualNode.aiConfig
                            });
                        }
                        ...
                        if (actualNode.pythonResultName && actualNode.pythonCode) {
                            ...
                            let nodeData = actualNode.pythonPreviewResult;
                            if (nodeData && typeof nodeData === 'object' && !Array.isArray(nodeData) && nodeData.data) {
                        ...
                        // Check for AI result output (prompt is the key indicator of an AI node)
                        if (actualNode.aiConfig?.prompt && actualNode.aiConfig?.outputName) {
                            // Don't add duplicate if already added as SQL with the same name
                            const aiName = actualNode.aiConfig.outputName;
                            const alreadyAdded = ancestorItems.some(
                                a => a.name === aiName && a.nodeId === actualNode.id
                            );
                            if (!alreadyAdded) {
                                ancestorItems.push({
                                    path: nodePath,
                                    ...
                                    pipelineDependencies: resolveDependencies(actualNode),
                                    data: actualNode.aiConfig.lastResult !== undefined
                                        ? (Array.isArray(actualNode.aiConfig.lastResult)
    ...
    const getLinkedNodesTables = useMemo(() => {
        return (currentPath: string): { name: string, nodeName?: string, nodeId?: string, connectorId?: string, sqlQuery?: string, isPython?: boolean, pythonCode?: string, pythonOutputType?: 'table' | 'variable' | 'chart', pipelineDependencies?: { tableName: string; query?: string; isPython?: boolean; pythonCode?: string; connectorId?: string; nodeId?: string; writesToDatabase?: boolean; nodeName?: string }[], writesToDatabase?: boolean, sqlExportTargetTableName?: string, sqlExportTargetConnectorId?: string, sqlExportSourceTables?: string[], data?: any[] }[] => {
            const linkedTables: { name: string, nodeName?: string, nodeId?: string, connectorId?: string, sqlQuery?: string, isPython?: boolean, pythonCode?: string, pythonOutputType?: 'table' | 'variable' | 'chart', pipelineDependencies?: { tableName: string; query?: string; isPython?: boolean; pythonCode?: string; connectorId?: string; nodeId?: string; writesToDatabase?: boolean; nodeName?: string }[], writesToDatabase?: boolean, sqlExportTargetTableName?: string, sqlExportTargetConnectorId?: string, sqlExportSourceTables?: string[], data?: any[] }[] = [];
                            ...
                            if (parentItem) {
                                ...
                                if (parentNode.sqlResultName) {
                                    ...
                                    linkedTables.push({
                                        ...
                                        data: parentNode.sqlPreviewData
                                    });
                                }
                                ...
                                if (parentNode.pythonResultName && parentNode.pythonCode) {
                                    ...
                                    let nodeData = parentNode.pythonPreviewResult;
                                    if (nodeData && typeof nodeData === 'object' && !Array.isArray(nodeData) && nodeData.data) {
                                        nodeData = nodeData.data;
                                    }
                                        ...
                                        <div className={cn(
                                            ...
                                            <div className="flex-grow min-w-0 flex flex-col justify-center">
                                                ...
                                                <div className="flex items-center gap-1 mt-1">
                                                    ...
                                                    {scheduledNodeIds.has((item.node as any).id) && <Clock className="h-3 w-3 text-red-500" />}
