...
function traverseAndExtract(node: DecisionNode, extracted: Variable[]) {
    if (typeof node === 'object' && node !== null && node.question && node.options) {
        ...
        Object.values(node.options).forEach(childNode => {
            if (typeof childNode === 'object' && childNode !== null && !('ref' in childNode)) {
...
export default function ConsolidateVariablesDialog({
    ...
    useEffect(() => {
        if (isOpen && tree && dbVariables) {
            ...
            // 1.5 Detect and Handle ID Collisions
            const seenOrphanIds = new Set<string>();
            extractedVars.forEach(v => {
                if (!v.id) return;

                const dbMatch = dbVariables.find(d => d.id === v.id);
                if (dbMatch) {
                    // Collision with Existing DB Variable (Imposter)
                    // If names mismatch significantly, it means this tree node has a wrong ID.
                    if (dbMatch.name.trim().toLowerCase() !== v.name.trim().toLowerCase()) {
                        v.id = undefined; // Treat as new
                    }
                } else {
                    // Collision among Orphans
                    if (seenOrphanIds.has(v.id)) {
                        v.id = undefined; // Duplicate ID usage, treat as new
                    } else {
                        seenOrphanIds.add(v.id);
                    }
                }
            });

            // 2. Filter out variables that are already standardized (i.e., have an ID present in dbVariables)
            // Fix: Also include variables that HAVE an ID but that ID is NOT in dbVariables (orphans)
            const nonStandardizedVars = extractedVars.filter(v => {
                if (!v.id) return true; // Truly new variable
                const existsInDb = dbVariables.some(dbV => dbV.id === v.id);
                return !existsInDb; // Orphaned variable (has ID but not in DB)
            });
        ...
        const approvedActions = proposals
            ...
            .map(p => {
                ...
                return {
                    ...
                    dbVarId: p.type === 'merge' && p.dbVariable ? p.dbVariable.id : (p.treeVariable.id || undefined),
