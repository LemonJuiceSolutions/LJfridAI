...
import { Check, Send, Printer, ClipboardList, Play, ChevronRight, Info, Wind, AlertTriangle, CheckCircle, Percent, Package } from 'lucide-react';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
const SheetDialog = ({ job, isOpen, onOpenChange }: { job: Job | null, isOpen: boolean, onOpenChange: (open: boolean) => void }) => {
    ...
    const documentTitle = isExternal ? 'Ordine di Conto Lavoro / DDT' : 'Scheda di Lavorazione Stiro';
...
export default function StiroWidget() {
  ...
  return (
    ...
    <CardHeader>
      <CardTitle>Reparto Stiro</CardTitle>
      <CardDescription>Assegna e traccia i lotti di stiro per lavorazioni interne ed esterne.</CardDescription>
