...
import { getPipelines, savePipeline, deletePipeline } from '@/actions/pipelines';
...
export default function PipelinesWidget() {
    ...
    useEffect(() => {
        ...
        const loadState = async () => {
            setIsLoading(true);
            try {
                // Load pipelines and connections in parallel
                const [loadedPipelines, loadedConnections] = await Promise.all([
                    getPipelines(),
                    getConnections()
                ]);

                if (loadedPipelines && loadedPipelines.length > 0) {
                    // Start with empty then populate
                    // Need to parse Json fields
                    const parsedPipelines = loadedPipelines.map((p: any) => ({
                        ...p,
    ...
    const savePipelineLocal = useCallback(async (pipeline: any) => {
        try {
            await savePipeline(pipeline);
        } catch (e) {
            console.error("Failed to save pipeline", e);
            toast({ variant: "destructive", title: "Errore", description: "Impossibile salvare la pipeline" });
        }
    }, [toast]);

    // Use a targeted update function instead of modifying the whole array
    const updatePipeline = useCallback((pipelineId: string, updater: (draft: any) => void) => {
        setPipelines(currentPipelines => {
            const pipelineIndex = currentPipelines.findIndex(p => p.id === pipelineId);
            if (pipelineIndex === -1) return currentPipelines;

            const updatedPipelines = [...currentPipelines];
            const basePipeline = updatedPipelines[pipelineIndex];

            // Apply produce to the specific pipeline
            const newPipeline = produce(basePipeline, updater);
            updatedPipelines[pipelineIndex] = newPipeline;

            // Save just this pipeline
            savePipelineLocal(newPipeline);
    ...
    const handleDeleteEdge = (pipelineId: string, edgeToDelete: any) => {
        updatePipeline(pipelineId, draft => {
            draft.edges = draft.edges.filter((e: any) =>
                !(e.from === edgeToDelete.from && e.to === edgeToDelete.to && e.fromPort === edgeToDelete.fromPort)
            );

            const toNode = draft.nodes[edgeToDelete.to];
            if (toNode) {
    ...
    const handleAddEdge = (fromNodeId: string, fromPort: number, toNodeId: string) => {
        // Need to find pipeline ID. But `updatePipeline` needs it.
        // `getActivePipelineId` requires currentPipelines.
        const pipelineId = getActivePipelineId(fromNodeId, pipelines);
        if (!pipelineId) return;

        updatePipeline(pipelineId, draft => {
            if (fromNodeId === toNodeId) return;

            const fromNode = draft.nodes[fromNodeId];
            const toNode = draft.nodes[toNodeId];
            if (!fromNode || !toNode) return;

            // Remove existing incoming edges to the target node
            draft.edges = draft.edges.filter((e: any) => e.to !== toNodeId);

            // Add the new edge
            draft.edges.push({ from: fromNodeId, to: toNodeId, fromPort });

            // Update parentNodes reference on the target node
            draft.nodes[toNodeId].parentNodes = [{
                id: fromNodeId,
                name: fromNode.name,
                outputs: fromNode.outputs || [{ name: 'Trigger', type: 'trigger' }]
            }];
            draft.nodes[toNodeId].inputId = `${fromNodeId}-out-${fromPort}`;
        });
    };

    const handleAddNode = (pipelineId: string, parentNodeId: string, portIndex: number) => {
        updatePipeline(pipelineId, draft => {
            const parentNode = draft.nodes[parentNodeId];
            ...
            draft.nodes[newNodeId] = newNode;
            draft.edges.push({ from: parentNodeId, to: newNodeId, fromPort: portIndex });
        });
    };

    const handleDeleteNode = (pipelineId: string, nodeId: string) => {
        updatePipeline(pipelineId, draft => {
            if (['start', 'end'].includes(draft.nodes[nodeId].type)) {
                // Should show toast but inside callback is tricky? Actually it's fine
                // But better to check before calling update.
                // For simplicity, checking inside.
                return; // Just return, logic handled outside if needed or toast here?
            }

            delete draft.nodes[nodeId];

            draft.edges = draft.edges.filter((e: any) => e.from !== nodeId && e.to !== nodeId);
    ...
    const handleNodeSave = (updatedNodeWithContext: any) => {
        ...
        updatePipeline(pipelineId, draft => {
            if (draft.nodes && draft.nodes[updatedNode.id]) {
                draft.nodes[updatedNode.id] = {
                    ...draft.nodes[updatedNode.id],
                    ...updatedNode
                };
            }
    ...
    const handleAddPipeline = async (name: string, description: string) => {
        ...
        try {
            ...
            console.error(e);
            toast({ variant: "destructive", title: "Errore", description: "Impossibile creare pipeline" });
        }
    };
