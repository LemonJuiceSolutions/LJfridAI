...
import { ref, listAll, getDownloadURL, uploadBytes, deleteObject, type StorageReference } from 'firebase/storage';
...
const listAllFiles = async (folderRef: StorageReference): Promise<StorageReference[]> => {
    ...
    for (const folder of res.prefixes) {
        const subfolderFiles = await listAllFiles(folder);
...
export default function StoragePage() {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
        ...
        const allFileRefs = await listAllFiles(rootRef);

        const fileDetails = await Promise.all(
            allFileRefs.map(async (itemRef) => {
        ...
        if (err.code === 'storage/unauthorized') {
            errorMessage = "Non autorizzato. Controlla le regole di sicurezza di Firebase Storage.";
        } else if (err.code === 'storage/retry-limit-exceeded') {
            errorMessage = 'Limite di tentativi superato. Controlla la tua connessione e la configurazione di Firebase (storageBucket, CORS).';
        }
