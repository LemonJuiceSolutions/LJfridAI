...
import { getTreesAction } from '../actions';
...
export default function PipelinePage() {
    ...
    const handleDownloadAll = async () => {
        if (trees.length === 0) {
            toast({
                title: "Nessuna Pipeline da Esportare",
                description: "Crea una pipeline prima di esportare."
            });
            return;
        }
        ...
        try {
            ...
            a.href = url;
            a.download = 'like-ai-said-pipeline.json';
            ...
            toast({
                ...
                description: "Tutte le pipeline sono state scaricate."
            });
    ...
    const handleDeleteSingle = async () => {
        ...
        try {
            ...
            if (result.success) {
                toast({
                    title: 'Pipeline Eliminata',
                    description: `La pipeline "${treeToDelete.name}" è stata rimossa con successo.`,
                });
                await fetchTrees();
            } else {
    ...
    const handleDeleteAll = async () => {
        ...
        try {
            ...
            if (result.success) {
                toast({
                    title: 'Tutte le Pipeline Eliminate',
                    description: 'Tutte le pipeline sono state rimosse.',
                });
                await fetchTrees();
            } else {
                        ...
                        <CardHeader className="space-y-4">
                            <div className="flex flex-row items-start justify-between gap-4">
                                <div>
                                    ...
                                    <CardDescription>
                                        Cerca o fai clic su una pipeline per visualizzarla e modificarla.
                                    </CardDescription>
                                </div>
                                <div className="flex gap-2 shrink-0">
                                    <Link href="/create?type=PIPELINE">
                        ...
                        <CardContent>
                            {isLoading ? (
                                    ...
                                    {filteredTrees.length === 0 && (
                                        <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed bg-card p-8 text-center min-h-[200px]">
                                            ...
                                            <p className="mt-2 text-muted-foreground">
                                                Nessuna pipeline corrisponde alla tua ricerca &quot;{searchQuery}&quot;.
                                            </p>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed bg-card p-8 text-center min-h-[200px]">
                                    ...
                                    <h2 className="mt-6 text-xl font-semibold">Ancora Nessuna Pipeline</h2>
                                    <p className="mt-2 text-muted-foreground">
                                        Crea la tua prima pipeline iniziando da qui.
                                    </p>
                    ...
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            {dialogState === 'delete-all' ?
                                "Questa azione non può essere annullata. Questo eliminerà permanentemente TUTTE le pipeline." :
                                `Questa azione non può essere annullata. Questo eliminerà permanentemente la pipeline "${treeToDelete?.name}".`
                            }
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
