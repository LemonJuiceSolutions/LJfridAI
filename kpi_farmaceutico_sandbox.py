"""
Analisi KPI - Settore Farmaceutico Italiano (Sandbox)
"""
...
# ── CONFIG ──────────────────────────────────────────────────────────────────
INPUT_FILE = "/Users/manuelezanoni/Desktop/VisualStudio/FridAI/public/documents/rating_farmaceutico_finale.xlsx"
SANDBOX_MAX_FILTER_ROWS = 5000  # Cap per dimensione HTML in sandbox
SANDBOX_MAX_DD_COMPANIES = 20   # Max aziende per livello drill-down
...
# ── 4. EXPORT EXCEL (skipped in sandbox) ────────────────────────────────────
...
# Costruisci array compatto (capped per sandbox)
...
_df_last_sample = df_last.head(SANDBOX_MAX_FILTER_ROWS) if len(df_last) > SANDBOX_MAX_FILTER_ROWS else df_last
print(f"  FILTER_RAW: campione {len(_df_last_sample):,} su {len(df_last):,} aziende")
for _, row in _df_last_sample.iterrows():
...
# ── TREND DATA: multi-anno per filtri (limitato al campione) ──
...
_active_sample = df[df["status"] == "Active"]
_active_sample = _active_sample[_active_sample["ID"].isin(set(_fr_company_ids))]
for _, row in _active_sample.iterrows():
    cid = row.get("ID", "")
